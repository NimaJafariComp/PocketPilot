...
export function Sparkline({ points, width = 240, height = 56, color, fillColor }: SparklineProps) {
  ...
  return (
    <Svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`}>
      ...
      <Path
        d={path}
        fill="none"
        stroke={color || colors.goalPalette.stroke}
        strokeWidth="3"
        strokeLinecap="round"
      />
      {lastPoint ? (
        <>
          <Circle
            ...
            cy={lastPoint.y}
            r="5"
            fill={fillColor || colors.goalPalette.fill}
          />
          <Circle
            cx={lastPoint.x}
            cy={lastPoint.y}
            r="3"
            fill={color || colors.goalPalette.stroke}
          />
        </>
      ) : null}
    </Svg>
