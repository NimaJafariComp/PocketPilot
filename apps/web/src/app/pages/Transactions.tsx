...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  Groceries:
    "border-yellow-500/25 bg-yellow-500/12 text-yellow-700 dark:border-yellow-400/25 dark:bg-yellow-400/10 dark:text-yellow-300",
  Dining:
    "border-orange-500/25 bg-orange-500/12 text-orange-700 dark:border-orange-400/25 dark:bg-orange-400/10 dark:text-orange-300",
  Transport:
    "border-blue-500/25 bg-blue-500/12 text-blue-700 dark:border-blue-400/25 dark:bg-blue-400/10 dark:text-blue-300",
  Transportation:
    "border-blue-500/25 bg-blue-500/12 text-blue-700 dark:border-blue-400/25 dark:bg-blue-400/10 dark:text-blue-300",
  Entertainment:
    "border-violet-500/25 bg-violet-500/12 text-violet-700 dark:border-violet-400/25 dark:bg-violet-400/10 dark:text-violet-300",
  Shopping:
    "border-pink-500/25 bg-pink-500/12 text-pink-700 dark:border-pink-400/25 dark:bg-pink-400/10 dark:text-pink-300",
  Health:
    "border-emerald-500/25 bg-emerald-500/12 text-emerald-700 dark:border-emerald-400/25 dark:bg-emerald-400/10 dark:text-emerald-300",
  Income: "border-success/25 bg-success/12 text-success",
  Utilities:
    "border-sky-500/25 bg-sky-500/12 text-sky-700 dark:border-sky-400/25 dark:bg-sky-400/10 dark:text-sky-300",
  Bills:
    "border-red-500/25 bg-red-500/12 text-red-700 dark:border-red-400/25 dark:bg-red-400/10 dark:text-red-300",
  Uncategorized: "border-border bg-muted text-muted-foreground",
};

function CategoryBadge({ category }: { category: string }) {
  const colors =
    CATEGORY_COLORS[category] ?? "bg-secondary text-secondary-foreground border-border";
  return (
    <span
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (transactionToSave.category === "Uncategorized") {
        ...
        selectedTransaction &&
        transactionToSave.category !== selectedTransaction.category
      ) {
        transactionToSave = {
          ...transactionToSave,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {uncategorizedCount > 0 && (
            <Badge
              variant="outline"
              className="w-fit gap-1.5 border-warning/25 bg-warning/12 text-warning-foreground"
            >
                ...
                <TableBody>
                  {paginatedTransactions.map((transaction) => (
                    ...
                    >
                      ...
                      <TableCell
                        className={`text-right font-semibold text-sm pr-5 py-3.5 ${transaction.amount < 0 ? "text-destructive" : "text-success"}`}
                      >
            ...
            <>
              <div className="flex h-full flex-col">
                ...
                <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
                  <div
                    className={`rounded-2xl border px-5 py-4 text-center ${editedTransaction.amount < 0 ? "border-destructive/20 bg-destructive/10" : "border-success/20 bg-success/12"}`}
                  >
                    <p
                      className={`text-2xl font-bold tracking-tight ${editedTransaction.amount < 0 ? "text-destructive" : "text-success"}`}
                    >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setEditedTransaction({ ...editedTransaction, merchant: e.target.value })
                      }
                      className="h-10"
                    />
                  </div>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete <strong>{selectedTransaction?.merchant}</strong>? This
