...
export function buildRagDocuments({
  ...
  const merchantSummaryDocs: InsightRagDocument[] = Array.from(
    transactions
      .reduce(
        (acc, transaction) => {
          ...
          existing.count += 1;
          existing.latestDate =
            existing.latestDate > transaction.date ? existing.latestDate : transaction.date;
    ...
    .map((merchant) => ({
      ...
      text: [
        ...
        `TopCategories: ${
          Array.from(merchant.categories.entries())
            .sort((a, b) => b[1] - a[1])
            .slice(0, 3)
  ...
  const transactionDocs: InsightRagDocument[] = transactionDocCandidates.map((transaction) => {
    ...
    const monthIndex = Number.isNaN(transactionDate.getTime())
      ? null
