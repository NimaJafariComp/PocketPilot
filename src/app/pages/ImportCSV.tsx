...
export function ImportCSV() {
  ...
  const handleImport = () => {
    const transactions = parsedData.map(row => {
      let amount = parseFloat(row[mapping.amount].replace(/[^0-9.-]/g, ''));
      
      // If amount is positive but should be negative (expense), make it negative
      if (amount > 0 && !row[mapping.merchant].toLowerCase().includes('payment')) {
        amount = -amount;
      }
              ...
              <Table>
                ...
                <TableBody>
                  {parsedData.slice(0, 20).map((row, idx) => (
                    <TableRow key={idx}>
                      <TableCell>{format(new Date(row[mapping.date]), 'MMM dd, yyyy')}</TableCell>
